...
pub fn choose(rows: &[RowInfo], ctx: &Context, fallback: usize, rng: &mut Rng) -> usize {
    let pool: Vec<&RowInfo> = rows.iter().filter(|r| r.selectable()).collect();
    if pool.is_empty() {
        return fallback;
    }

    // 1. Cue overrides, in the priority order of spec §11.3.
    if ctx.building {
    ...
    if ctx.boundary {
        ...
        let hits: Vec<&RowInfo> = pool
            .iter()
            .copied()
            .filter(|r| r.energy.is_some_and(|e| e >= 0.6))
            .collect();
    ...
    // 2. Labels, when the score has them. Enrichment, not a requirement.
    ...
    // 3. Energy proximity. Rows that declare no energy stay eligible: a sheet
    ...
    // 4. No immediate repeats — unless that would leave nothing to pick from.
    ...
    // 5. Weighted random, favouring rows whose energy sits closest to the music's.
    let energy = ctx.energy;
    weighted(&fresh, |r| match (energy, r.energy) {
